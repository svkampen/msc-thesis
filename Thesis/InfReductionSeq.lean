...
-/
private lemma aux_elem' (f: ℕ → α) (hf: inf_reduction_seq r⁺ f) (n) (hn: n > 0):
...
-/
private lemma aux_elem (f: ℕ → α) (hf: inf_reduction_seq r⁺ f) (n: ℕ) (hn: n > 0):
...
private lemma trans_seq_inf_reduction_seq: inf_reduction_seq r⁺ (trans_seq f hgs) := by
  ...
  rw [this]
  have := reflTransGen_iff_eq_or_transGen.mp <| hf c
  tauto
