...
private lemma trans_chain'.nonempty {h: r⁺ a b}: (trans_chain' h) ≠ [] := by
  have := (spec h).1
  intro heq
...
-/
private noncomputable def inf_trans_lists: ℕ → List α
| n => trans_chain' (hf n (by simp))

/-- Each of the lists in `inf_trans_lists f hf` is nonempty. -/
private lemma inf_trans_lists.nonempty {f: ℕ → α} {hf: reduction_seq r⁺ ⊤ f}:
...
-/
private lemma aux_elem' (n) (hn: n > 0):
...
-/
private lemma aux_elem (n: ℕ) (hn: n > 0):
...
private lemma seq_inf_reduction_seq:
...
-/
private noncomputable def trans_idx_step (n: ℕ): ℕ :=
  choose (htsg n) + 1
