...
variable {α: Type*} {r: Rel α α} {a b: α}
...
variable (f: ℕ → α) (hf: reduction_seq r⁺ ⊤ f)
...
-/
lemma trans_chain'.spec (h: r⁺ a b):
...
lemma trans_chain'.nonempty {h: r⁺ a b}: (trans_chain' h) ≠ [] := by
  have := (spec h).1
  intro heq
...
-/
noncomputable def inf_trans_lists: ℕ → List α
| n => trans_chain' (hf n (by simp))

/-- Each of the lists in `inf_trans_lists f hf` is nonempty. -/
lemma inf_trans_lists.nonempty {f: ℕ → α} {hf: reduction_seq r⁺ ⊤ f}:
  ∀n, (inf_trans_lists f hf n) ≠ []
...
-/
def aux (list_idx: ℕ) (elem_idx: ℕ) : α :=
...
/-
`aux (inf_trans_lists f hf) inf_trans_lists.nonempty 0` is an infinite reduction sequence
with respect to `r`.

Parts of this lemma and its proof are due to Edward van de Meent on the Lean Zulip.
-/
lemma aux_inf_reduction_seq:
    reduction_seq r ⊤ (aux (inf_trans_lists f hf) (inf_trans_lists.nonempty) 0) := by
  generalize 0 = list_idx
  intro elem_idx
  simp
  induction list_idx, elem_idx using aux.induct (inf_trans_lists f hf) inf_trans_lists.nonempty with
  | case1 list_idx elem_idx h =>
    have hl₁ := trans_chain'.spec <| hf list_idx (WithTop.coe_lt_top _)
    have hl₂ := trans_chain'.spec <| hf (list_idx + 1) (WithTop.coe_lt_top _)

    simp [List.chain_iff_get] at hl₁ hl₂
    rw [aux, dif_pos h, aux]

    split_ifs with h₂
    · simp [inf_trans_lists] at h h₂ ⊢
      apply hl₁.2.2
      omega
    · obtain ⟨rfl⟩: elem_idx = (inf_trans_lists f hf list_idx).length - 1 := by
        simp at h₂
        omega
      have hlt: 0 < (inf_trans_lists f hf (list_idx + 1)).length := by
        simp [List.length_pos, inf_trans_lists, trans_chain'.nonempty]
      rw [aux, dif_pos (by omega)]
      ...
      omega
  | case2 list_idx elem_idx h hlt ih =>
    clear hlt
    rw [aux, dif_neg h]
    convert ih using 1
    rw [aux, dif_neg (by omega)]
    congr
    omega
...
lemma seq_inf_reduction_seq:
  ...
  | succ n =>
    ...
    simp_rw [seq]
    exact (aux_inf_reduction_seq f hf).inf_step n
...
section flatten_refl_trans_seq
...
variable {f: ℕ → α} (hseq: reduction_seq r ⊤ f)
...
def transitive_step_guarantee (hseq: reduction_seq r ⊤ f) :=
  ∀n, ∃m ≥ n, f m ≠ f (m + 1) ∧ (∀m' ∈ Set.Icc n m, f n = f m')

lemma tsg_of_not_degenerate
    (hndeg: ¬hseq.degenerate):
...
-/
noncomputable def trans_idx_step (n: ℕ): ℕ :=
  choose (htsg n) + 1
...
lemma trans_idxs_tends_to_infty (n: ℕ): ∃k, n < trans_idxs hf htsg k := by
  induction n with
  | zero =>
...
lemma trans_seq_inf_reduction_seq: reduction_seq r⁺ ⊤ (trans_seq hf htsg) := by
  rintro n -
  simp only [trans_seq, Function.iterate_succ_apply']

  have ⟨hc₁, hc₂, hc₃⟩ := choose_spec (htsg ((trans_idx_step hf htsg)^[n] 0))

  have hstep: r⁺ (f ((trans_idxs hf htsg (n + 1)) - 1)) (f (trans_idxs hf htsg (n + 1))) := by
    have ⟨hc₁, hc₂, hc₃⟩ := choose_spec (htsg ((trans_idx_step hf htsg)^[n] 0))
    let hstep := hf.inf_step ((trans_idxs hf htsg (n + 1)) - 1)
    simp [trans_idxs, Function.iterate_succ_apply', trans_idx_step] at hstep ⊢
    refine (reflTransGen_iff_eq_or_transGen.mp <| hstep).resolve_left ?_
    simpa using hc₂.symm

  have heq: f (trans_idxs hf htsg n) = f ((trans_idxs hf htsg (n + 1)) - 1) := by
    simp [trans_idxs, Function.iterate_succ_apply', trans_idx_step] at hc₁ ⊢
    apply hc₃
    simp [hc₁]
...
lemma trans_seq_contains_elems (n): ∃m, (trans_seq hf htsg m) = f n := by
...
-/
lemma exists_inf_regular_seq_of_not_degenerate (hnd: ¬hf.degenerate):
    ∃f', reduction_seq r ⊤ f' ∧
         (∀n, ∃m, f n = f' m) ∧
         f 0 = f' 0 := by
...
-/
lemma finite_of_degenerate (f: ℕ → α) (hf: reduction_seq r∗ ⊤ f) (hdg: hf.degenerate):
...
-/
lemma finite_seq_flatten (N: ℕ) (f: ℕ → α) (hrs: reduction_seq r∗ N f):
    ∃N': ℕ, ∃f', reduction_seq r N' f' ∧ f' 0 = f 0 ∧ ∀n ≤ N, ∃m < N' + 1, f' m = f n := by
...
`a₀ -> ... -> a₁ -> ... -> a₂ -> ...`
-/
lemma regular_seq_of_rt_seq (f: ℕ → α) (hf: reduction_seq r∗ ⊤ f):
    ∃N f', reduction_seq r N f' ∧ (∀n, ∃(m: ℕ) (_: m < N + 1), f n = f' m) ∧ f 0 = f' 0 := by
